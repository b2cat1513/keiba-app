...
st.title("🏆 ジェニーAI予想ver1.15.3（スマホ単一画像アップロード版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    for row_idx in range(8):
        ...
        # 騎手は狭い列と少し広い補助列の両方をマスター照合。
        jockey = _best_master_match(detail_text, jockey_candidates, 0.34)
        if not jockey or jockey == "(未選択)":
            jockey = _best_master_match(jockey_text, jockey_candidates, 0.32)
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.15.3-行分割＋詳細列OCR)",
        })
...
with tab_img:
    ...
    st.info("📱 Ver1.15.3：8行分割方式を維持したまま、U指数を小数1桁に補正し、騎手・斤量の専用OCRを強化しました。複数枚は順番に処理してください。")
