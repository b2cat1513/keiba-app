...
# ⚙️ アプリ初期設定 & レイアウト
# ==========================================
st.set_page_config(page_title="ジェニーAI予想ver1.15.0", layout="wide")
st.title("🏆 ジェニーAI予想ver1.15.1（スマホ単一画像アップロード版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    """ウマニティ画像をOCRし、馬名とU指数を画面の縦順で復元する。

    Ver1.15.1:
    - 座標OCRの右端判定に依存しすぎない。
    - OCR原文に残る「馬名（カタカナ）」と「U指数＋丸数字順位」を縦順で抽出。
    - 例: 94.4① / 91.3⑮ / 98.5② のような行をU指数として強く採用。
    - 馬名とU指数を上から順に対応させ、1番・2番・3番...と採番する。
    - Enns / EZRA / ERnSz / Ht4 などの英字OCRノイズは馬名に採用しない。
    """
    ...
    try:
        ...
        ocr_text = max(
            text_candidates,
            key=lambda s: len(re.findall(r"[一-龥ぁ-んァ-ヶーA-Za-z0-9]", s))
        ) if text_candidates else raw_text
    ...
    def _katakana_name(text):
        s = normalize_horse_name(_ocr_clean_line(text))
        if not s:
            return ""
        s = re.sub(r"[^ァ-ヶーヴー]", "", s)
    ...
    # A. OCR原文の縦順を利用する
    # --------------------------------------------------------------
    lines = [_ocr_clean_line(x) for x in str(ocr_text).splitlines()]
    lines = [x for x in lines if x]

    # 丸数字。U指数の右に表示される順位を識別用アンカーとして使う。
    circled_chars = "①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱"
    circled_re = re.compile(
        rf"(?<!\d)((?:8\d|9\d|10\d)(?:[.,]\d{{1,2}})?)\s*[{circled_chars}]"
    )
    ...
    for line_no, line in enumerate(lines):
        name = _katakana_name(line)
        if name:
            # 同じ名前の重複OCRを近接位置で除外
            if not ordered_names or ordered_names[-1][1] != name:
                ordered_names.append((line_no, name))

        m = circled_re.search(line)
        if m:
            try:
                val = float(m.group(1).replace(",", "."))
            except Exception:
                val = None
            if val is not None and 80.0 <= val <= 110.0:
                ordered_u.append((line_no, val))

    # OCRによって丸数字が別トークンになった場合の補助。
    # 「指数値」と「丸数字」が同一行または非常に近い行にあるケースを拾う。
    if len(ordered_u) < 3:
        for i, line in enumerate(lines):
            nums = re.findall(r"(?<!\d)((?:8\d|9\d|10\d)(?:[.,]\d{1,2})?)(?!\d)", line)
            if not nums:
                continue
            near = " ".join(lines[max(0, i-1):min(len(lines), i+2)])
            if not re.search(rf"[{circled_chars}]", near):
                continue
            for num in nums:
                try:
                    ...
                    continue
                if 80.0 <= val <= 110.0 and not any(abs(val-v) < 0.001 and abs(i-j) <= 1 for j,v in ordered_u):
                    ordered_u.append((i, val))
        ordered_u.sort(key=lambda x: x[0])

    # 馬名候補を、各U指数の直前にあるカタカナ馬名から決める。
    # 同じ馬名は一度だけ使用。
    text_records = []
    used_names = set()
    last_u_line = -1

    for seq_gate, (u_line, u_val) in enumerate(ordered_u, start=1):
        # 基本は「前のU指数より後 ～ 今回U指数より前」にある最後の馬名。
        candidates = [
            (ln, nm) for ln, nm in ordered_names
            if last_u_line < ln < u_line and nm not in used_names
        ]

        # OCR順が少し前後しても、直前12行まで許容。
        if not candidates:
            candidates = [
                (ln, nm) for ln, nm in ordered_names
                if max(-1, u_line - 12) <= ln < u_line and nm not in used_names
            ]
        ...
        _, horse_name = candidates[-1]
        used_names.add(horse_name)

        # U指数周辺から補助情報を抽出
        block_start = max(0, u_line - 6)
        block_end = min(len(lines), u_line + 6)
        block_text = " ".join(lines[block_start:block_end])

        sex_age = _parse_sex_age(block_text)
        weight_m = re.search(r"(?<!\d)(4[8-9](?:\.5)?|5\d(?:\.5)?|6[0-2](?:\.5)?)(?!\d)", block_text)
        odds_m = re.search(r"(\d{1,3}(?:\.\d+)?)\s*倍", block_text)
        jockey_candidates = [x for x in JOCKEY_MASTER if x != "その他（自由手入力）"]
        trainer_candidates = [x for x in TRAINER_OPTIONS if x != "(未選択)"]
        jockey = _best_master_match(block_text, jockey_candidates, 0.40)
        trainer = _best_master_match(block_text, trainer_candidates, 0.40)
        ...
        text_records.append({
            ...
            "厩舎": trainer,
            "単勝": float(odds_m.group(1)) if odds_m else None,
            "人気": None,
            "U指数": float(u_val),
            "取得元": "ウマニティ画像(Ver1.15.1-縦順テキストOCR)",
        })
        last_u_line = u_line

    # 3頭以上復元できた場合は、ノイズを含みやすい旧fallbackを混ぜず
    # 新しい縦順結果を優先する。
    ...
    name_tokens = []
    for t in tokens:
        name = _katakana_name(t["text"])
        if not name:
            continue
        if 0.12 * image_width <= _cx(t) <= 0.72 * image_width:
            tt = dict(t)
            tt["horse_name"] = name
    ...
    for t in tokens:
        ...
        if 80.0 <= val <= 110.0:
            tt = dict(t)
            tt["u_index"] = val
            numeric_tokens.append(tt)

    # 馬名のY位置を基準に最も近い80～110の数値を割り当てる。
    name_tokens.sort(key=_cy)
    coord_records = []
    used_num = set()

    for seq_gate, nt in enumerate(name_tokens, start=1):
        if seq_gate > 18:
            break
        ncy = _cy(nt)
        candidates = []
        for idx, ut in enumerate(numeric_tokens):
            if idx in used_num:
                continue
            dy = abs(_cy(ut) - ncy)
            if dy <= 120:
                # 同じ行なら右側数値を優先
                score = dy - (_cx(ut) / max(image_width, 1)) * 20
                candidates.append((score, idx, ut))
        if not candidates:
            continue
        candidates.sort(key=lambda x: x[0])
        _, idx, ut = candidates[0]
        used_num.add(idx)
        ...
        coord_records.append({
            ...
            "単勝": None,
            "人気": None,
            "U指数": float(ut["u_index"]),
            "取得元": "ウマニティ画像(Ver1.15.1-座標fallback)",
        })
...
with tab_img:
    ...
    st.info("📱 Ver1.15.1：OCR原文の「カタカナ馬名＋U指数丸数字」を縦順で対応させる方式に変更しました。複数枚は順番に処理してください。")
