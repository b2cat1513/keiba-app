...
st.title("🏆 ジェニーAI予想ver1.15.4（スマホ単一画像アップロード版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    for row_idx in range(8):
        ...
        # 騎手＋斤量列。
        # 0.45～0.80 は今回のウマニティ実画像で
        # 「騎手名 / 斤量 / ○週」が最も安定して読める範囲。
        detail_text = _ocr_crop(
            (int(w * 0.450), y1, int(w * 0.800), y2),
            lang="jpn+eng", psm=6
        )

        # 騎手名の先頭が左へはみ出す行（例: 太宰啓介）用の補助OCR。
        jockey_text = _ocr_crop(
            (int(w * 0.380), y1, int(w * 0.820), y2),
            lang="jpn+eng", psm=6
        )

        # Ver1.15.4: 騎手名専用の追加OCR
        jockey_text_narrow = _ocr_crop(
            (int(w * 0.505), y1, int(w * 0.735), y2),
            lang="jpn", psm=6
        )
        jockey_text_line = _ocr_crop(
            (int(w * 0.470), y1, int(w * 0.760), y2),
            lang="jpn", psm=7
        )
        jockey_text_sparse = _ocr_crop(
            (int(w * 0.440), y1, int(w * 0.790), y2),
            lang="jpn", psm=11
        )
        u_text = _ocr_crop(
            (int(w * 0.805), y1, w, y2),
        ...
        # 斤量は detail_text を最優先。
        # ここは「騎手 / 斤量 / ○週」だけを含む狭い列なので、
        # U指数や単勝オッズとの混同が起きにくい。
        weight = _extract_weight(detail_text)
        if weight is None:
            weight = _extract_weight(jockey_text)

        odds = _extract_odds(main_text)

        # Ver1.15.4: 複数の騎手専用OCR結果をマスター照合
        jockey = None
        for _jt, _threshold in [
            (jockey_text_narrow, 0.28),
            (jockey_text_line, 0.28),
            (jockey_text_sparse, 0.28),
            (detail_text, 0.32),
            (jockey_text, 0.30),
        ]:
            if not _jt:
                continue
            _cand = _best_master_match(_jt, jockey_candidates, _threshold)
            if _cand and _cand != "(未選択)":
                jockey = _cand
                break
        if not jockey:
            jockey = "(未選択)"
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.15.4-行分割＋詳細列OCR)",
        })
...
with tab_img:
    ...
    st.info("📱 Ver1.15.4：8行分割方式とU指数・斤量処理は維持したまま、騎手名だけを複数クロップ＋複数PSMで再OCRする方式に強化しました。複数枚は順番に処理してください。")
