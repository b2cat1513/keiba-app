...
st.title("🏆 ジェニーAI予想ver1.15.0（スマホ単一画像アップロード版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    """ウマニティ画像を座標OCR＋テキストOCRで解析する強化版。

    Ver1.15.0:
    - U指数は画面右端の指数列だけを採用し、斤量・オッズ等の誤検出を除外。
    - 馬名は日本語カタカナ表記を優先し、OCRノイズ英字や騎手名を馬名にしない。
    - 右端U指数の上からの並びを馬番1,2,3...として復元する。
    - 左端馬番OCRは補助情報としてのみ使用する。
    """
    ...
    def _katakana_name(text):
        """OCR文字列から馬名らしいカタカナ表記を返す。"""
        s = normalize_horse_name(_ocr_clean_line(text))
        if not s:
            return ""
        # JRAの日本語馬名表示は原則カタカナ。OCRノイズ記号は除去する。
        s = re.sub(r"[^ァ-ヶーヴー]", "", s)
        if len(s) < 3 or len(s) > 18:
            return ""
        if s in {"ウマニティ", "ニュース", "レース", "プロ予想", "コロシアム"}:
            return ""
        return s
    ...
    # 1) 右端U指数列を抽出
    # ------------------------------------------------------------------
    # まず数値候補を全て取得し、そのうち「右側」にあるものだけ残す。
    numeric_candidates = []
    for t in tokens:
        ...
        try:
            ...
            continue
        if not 80.0 <= val <= 110.0:
            continue
        tt = dict(t)
        tt["u_index"] = val
        numeric_candidates.append(tt)

    # ウマニティのU指数は画面右端。右側72%以降を基本条件にする。
    # 画像余白や端末差に備え、数値候補の最右端からも判定する。
    if numeric_candidates:
        rightmost_x = max(_cx(t) for t in numeric_candidates)
    else:
        rightmost_x = image_width
    u_tokens = [
        t for t in numeric_candidates
        if _cx(t) >= 0.70 * image_width
        and _cx(t) >= rightmost_x - 0.16 * image_width
    ]

    # 同じ指数の重複OCRをY座標で除去。
    u_tokens.sort(key=lambda t: (_cy(t), t["left"]))
    deduped = []
    for t in u_tokens:
        if deduped and abs(_cy(t) - _cy(deduped[-1])) < 32:
            # 同じ行で複数数値が出たら、より右側をU指数として優先。
            if _cx(t) > _cx(deduped[-1]):
                deduped[-1] = t
            continue
        deduped.append(t)
    u_tokens = deduped
    ...
    for t in tokens:
        ...
        if not name:
            continue
        cx = _cx(t)
        # 馬名列は画面左寄り～中央。右端指数列や最左端馬番列を除外。
        if not (0.16 * image_width <= cx <= 0.62 * image_width):
            continue
        tt = dict(t)
        tt["horse_name"] = name
    ...
    # ------------------------------------------------------------------
    # 3) 各U指数に最も近い馬名を対応付けて上から採番
    # ------------------------------------------------------------------
    records = []
    used_name_ids = set()
    for seq_gate, u_token in enumerate(u_tokens, start=1):
        if seq_gate > 18:
            break
        ucy = _cy(u_token)
        ...
        for idx, t in enumerate(name_tokens):
            if idx in used_name_ids:
                continue
            dy = _cy(t) - ucy
            # 馬名はU指数と同じ馬ブロック内。多少上下にずれても許容。
            if not (-190 <= dy <= 90):
                continue
            name = t["horse_name"]
            # 長めのカタカナ語・大きい文字を優先。U指数とのY距離も加味。
            score = (
                min(len(name), 12) * 8.0
                + t["height"] * 2.5
                - abs(dy) * 0.45
                - (_cx(t) / image_width) * 4.0
            )
            candidates.append((score, idx, name, t))

        if not candidates:
            continue
        candidates.sort(key=lambda x: x[0], reverse=True)
        _, chosen_idx, name, chosen_token = candidates[0]
        used_name_ids.add(chosen_idx)

        # 同一馬ブロックの補助項目を集める。
        row_tokens = [
            t for t in tokens
            if abs(_cy(t) - ucy) <= 150
        ]
        ...
        records.append({
            ...
            "人気": None,
            "U指数": float(u_token["u_index"]),
            "取得元": "ウマニティ画像(Ver1.15.0-日本語馬名+右端U指数)",
        })

    # ------------------------------------------------------------------
    # 4) テキストOCR fallbackと統合
    # ------------------------------------------------------------------
    # 今回の座標OCR結果を優先し、欠けた項目のみfallbackで補完する。
    by_gate = {}
    for rec in fallback:
        gate = int(rec.get("馬番", 0) or 0)
        if 1 <= gate <= 18:
            by_gate[gate] = dict(rec)
...
with tab_img:
    ...
    st.info("📱 Ver1.15.0：日本語馬名＋右端U指数で全頭抽出を強化しました。複数枚は順番に処理してください。")
