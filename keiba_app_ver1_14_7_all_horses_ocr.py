...
st.title("🏆 ジェニーAI予想ver1.14.7（スマホ単一画像アップロード版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    旧処理は「馬名とU指数が同じOCR行」「直後数行で馬番取得」が前提だった。
    スマホ縦長スクリーンショットでは列が別行に崩れやすいため、
    image_to_data の座標を使って U指数→馬名→馬番を同じ画面行として再構成する。
    """
    ...
    u_tokens = []
    for t in tokens:
        # 丸数字や余分な記号が付いても数値部分を拾う。
        m = re.search(r"(?<!\d)((?:8\d|9\d|10\d)(?:\.\d{1,2})?)(?!\d)", t["text"])
        if m:
            try:
                u = float(m.group(1))
            except Exception:
    ...
    for u_token in u_tokens:
        ...
        row_tokens = [
            t for t in tokens
            if abs((t["top"] + t["height"] / 2) - ucy) <= 175
        ]
    ...
    # 座標OCRと従来OCRを馬番単位で統合。座標OCRを優先し、足りない項目を旧処理で補う。
    by_gate = {}
...
with tab_img:
    ...
    st.info("📱 Ver1.14.7：画像は1枚ずつ選択して解析します。複数枚は順番に処理してください。")
