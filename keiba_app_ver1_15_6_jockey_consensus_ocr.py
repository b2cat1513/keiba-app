...
st.title("🏆 ジェニーAI予想ver1.15.6（騎手専用OCR強化版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    for row_idx in range(8):
        ...
        # Ver1.15.6: 騎手名は各行右上の「騎手専用帯」だけを複数条件でOCRする。
        # 画像では騎手名は概ね x=58〜79%、行高の上側 0〜35% にある。
        # 狭い帯・少し広い帯・疎な文字用の3系統を使い、単発誤認を減らす。
        rh = max(1, y2 - y1)
        jy1 = y1 + int(rh * 0.01)
        jy2 = y1 + int(rh * 0.35)

        jockey_text_tight = _ocr_crop(
            (int(w * 0.590), jy1, int(w * 0.790), jy2),
            lang="jpn", psm=7
        )
        jockey_text_wide = _ocr_crop(
            (int(w * 0.565), y1, int(w * 0.805), y1 + int(rh * 0.40)),
            lang="jpn", psm=7
        )
        jockey_text_sparse = _ocr_crop(
            (int(w * 0.575), jy1, int(w * 0.805), jy2),
            lang="jpn", psm=11
        )
        u_text = _ocr_crop(
            (int(w * 0.805), y1, w, y2),
        ...
        # Ver1.15.6: 完全一致/部分一致を最優先。曖昧一致は
        # 1位候補と2位候補の差が十分ある時だけ採用する。
        # 「中井裕二」→「菱田裕二」のような誤寄せを抑える。
        def _jockey_match_from_ocr(texts):
            ...
            if best_score >= 0.60 and best_overlap >= 2 and (best_score - second_score) >= 0.08:
                return best_name
            return "(未選択)"
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.15.6-騎手専用OCR強化)",
        })
