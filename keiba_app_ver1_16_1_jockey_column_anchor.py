...
st.title("🏆 ジェニーAI予想ver1.16.1（騎手列アンカーOCR版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    Ver1.16.1:
    - 画面全体OCRの文字順には依存しない。
    - 騎手欄だけは複数の横幅・縦位置・二値化条件で専用OCRし、JOCKEY_MASTERとの一致度を合議する。
    - 出馬表本体を8行に分割し、各行ごとに
    ...
    # Ver1.16.1: 騎手列を「画像全体の縦列」として先にOCRする。
    ...
    for row_idx in range(8):
        ...
        def _ocr_jockey_zone(x1r, x2r, y1r, y2r, psm=7, threshold=None, scale=6):
            try:
                ...
                return pytesseract.image_to_string(
                    gray, lang="jpn+eng", config=f"--oem 3 --psm {psm}"
                ).strip()
            except Exception:
                return ""
        ...
        jockey_ocr_texts = [
            _ocr_jockey_zone(0.54, 0.76, 0.00, 0.31, psm=7, threshold=None, scale=7),
            _ocr_jockey_zone(0.56, 0.79, 0.02, 0.34, psm=7, threshold=185, scale=7),
            _ocr_jockey_zone(0.58, 0.80, 0.00, 0.38, psm=6, threshold=165, scale=6),
            _ocr_jockey_zone(0.52, 0.80, 0.00, 0.40, psm=11, threshold=150, scale=6),
        ]
        u_text = _ocr_crop(
            (int(w * 0.805), y1, w, y2),
            lang="eng", psm=6
        ...
        def _jockey_match_from_ocr(texts):
            ...
            if wins:
                ...
                if wins[best] >= 2 and avg >= 0.58:
                    return best
                # 1方式だけなら非常に高い一致度を要求。
                if wins[best] == 1 and avg >= 0.88:
                    return best
        ...
        # 騎手欄で未選択だった場合のみ、詳細列全体を救済OCRとして使う。
        # 調教師・斤量が混ざるため判定閾値は高めに保つ。
        if jockey == "(未選択)":
            rescue = _normalize_jockey_ocr(detail_text)
            rescue_ranked = []
            for cand in jockey_candidates:
                sc, ov = _candidate_score(rescue, cand)
                rescue_ranked.append((sc, ov, cand))
            rescue_ranked.sort(key=lambda x: (x[0], x[1]), reverse=True)
            if rescue_ranked:
                sc, ov, cand = rescue_ranked[0]
                second = rescue_ranked[1][0] if len(rescue_ranked) > 1 else 0.0
                if sc >= 0.90 and ov >= 2 and sc - second >= 0.08:
                    jockey = cand

        # Ver1.16.1: 縦列OCRで得た騎手を最優先。
        # 個別切り出しが未選択でも、列全体で明瞭に読めた名前を救済できる。
        if row_idx in global_jockey_by_row:
            jockey = global_jockey_by_row[row_idx]

        trainer = _best_master_match(main_text, trainer_candidates, 0.40)
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.16.1-騎手列アンカーOCR版)",
        })
