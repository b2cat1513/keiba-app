...
st.title("🏆 ジェニーAI予想ver1.15.7（騎手欄固定座標OCR版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    for row_idx in range(8):
        ...
        # Ver1.15.7: 騎手欄を固定座標で読む。
        # ウマニティのスマホ出馬表では、騎手名は右側詳細列の「最上段」に固定表示される。
        # 調教師名はその下にあるため、縦方向を上側約24%までに絞り込む。
        # 横方向もU指数列へ入り込まないよう x=53.5〜71.5% に限定する。
        rh = max(1, y2 - y1)
        jy1 = y1 + int(rh * 0.00)
        jy2 = y1 + int(rh * 0.24)
        jx1 = int(w * 0.535)
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.15.7-騎手欄固定座標OCR)",
        })
