...
st.title("🏆 ジェニーAI予想ver1.16.3（騎手マルチバンドOCR強化版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    # Ver1.16.3: 騎手列を「画像全体の縦列」として先にOCRする。
    # 1頭ごとの小さな切り出しより、Tesseractが日本語の行を認識しやすい。
    # 読めた行はY座標で8頭の各行へ割り当て、後段の個別OCRより優先する。
    ...
    try:
        # 実画像では騎手名は概ね横52%～82%の列にある。
        gx1, gx2 = int(w * 0.645), int(w * 0.805)
        ...
        g_scale = 4
        g_gray = g_gray.resize((g_gray.width * g_scale, g_gray.height * g_scale))
        g_gray = g_gray.filter(ImageFilter.SHARPEN)

        # PSM 4/6/11を併用。各方式で行グループを作り、Y座標で馬行へ対応づける。
        global_votes = {}
        for gpsm in (4, 6, 11):
            try:
                ...
                df = df[df["text"].notna()]
                line_groups = df.groupby(["block_num", "par_num", "line_num"], sort=False)
                for _, grp in line_groups:
                    words = []
                    confs = []
                    for _, rr in grp.iterrows():
                        t = str(rr.get("text", "")).strip()
                        if not t or t == "nan":
                            continue
                        words.append(t)
                        try:
                            confs.append(float(rr.get("conf", -1)))
                        except Exception:
                            pass
                    if not words:
                        continue
                    line = "".join(words)
                    cand, score = _best_jockey_global(line)
                    if not cand or score < 0.58:
                        continue
                    # 元画像座標へ戻す。
                    top = float(grp["top"].min()) / g_scale + gy1
                    bottom = float((grp["top"] + grp["height"]).max()) / g_scale + gy1
                    cy = (top + bottom) / 2.0
                    row_idx_guess = int((cy - table_top) / row_h)
                    if not (0 <= row_idx_guess < 8):
                        continue
                    # 騎手は各馬行の上側約45%にある。週表示などの誤拾いを除外。
                    local_y = (cy - (table_top + row_h * row_idx_guess)) / row_h
                    if not (-0.12 <= local_y <= 0.48):
                        continue
                    avg_conf = sum(confs) / len(confs) if confs else 0.0
                    key = (row_idx_guess, cand)
                    global_votes[key] = global_votes.get(key, 0.0) + score + max(avg_conf, 0) / 500.0
            except Exception:
                continue
    ...
    for row_idx in range(8):
        ...
        # Ver1.16.3: 騎手は右側詳細列の「最上段」だけに限定する。
        # 左端を0.64未満へ広げると厩舎名（左列）が混入するため、完全に分離する。
        # Ver1.16.3: 騎手名は各行の上端ではなく、おおむね行高16〜50%付近にある。
        # 上端から大きく切ると罫線・前行の週表示が混ざるため、細い帯を少しずつずらして多数決する。
        jockey_ocr_texts = [
            _ocr_jockey_zone(0.600, 0.820, 0.14, 0.36, psm=7, threshold=None, scale=6),
            _ocr_jockey_zone(0.600, 0.820, 0.16, 0.40, psm=7, threshold=None, scale=6),
            _ocr_jockey_zone(0.600, 0.820, 0.18, 0.42, psm=6, threshold=None, scale=6),
            _ocr_jockey_zone(0.600, 0.820, 0.20, 0.44, psm=7, threshold=185, scale=6),
            _ocr_jockey_zone(0.600, 0.820, 0.22, 0.46, psm=7, threshold=None, scale=6),
            _ocr_jockey_zone(0.600, 0.820, 0.24, 0.48, psm=11, threshold=None, scale=6),
            _ocr_jockey_zone(0.600, 0.820, 0.26, 0.50, psm=7, threshold=None, scale=6),
            _ocr_jockey_zone(0.600, 0.820, 0.28, 0.52, psm=7, threshold=None, scale=6),
            _ocr_jockey_zone(0.600, 0.820, 0.30, 0.54, psm=7, threshold=None, scale=6),
        ]
        u_text = _ocr_crop(
            (int(w * 0.805), y1, w, y2),
            lang="eng", psm=6
        ...
        # Ver1.16.3: 未選択時の救済も「騎手専用列」からのみ行う。
        # detail_text全体は斤量・週表示・左側の厩舎が混ざるため騎手判定には使わない。
        if jockey == "(未選択)":
            rescue_texts = [
                _ocr_jockey_zone(0.585, 0.825, 0.15, 0.39, psm=6, threshold=None, scale=7),
                _ocr_jockey_zone(0.585, 0.825, 0.19, 0.45, psm=7, threshold=200, scale=7),
                _ocr_jockey_zone(0.585, 0.825, 0.23, 0.49, psm=11, threshold=None, scale=7),
                _ocr_jockey_zone(0.585, 0.825, 0.27, 0.53, psm=7, threshold=None, scale=7),
            ]
            rescue_ranked_all = []
        ...
        # Ver1.16.3: 縦列OCRで得た騎手を最優先。
        # 個別切り出しが未選択でも、列全体で明瞭に読めた名前を救済できる。
        if jockey == "(未選択)" and row_idx in global_jockey_by_row:
            jockey = global_jockey_by_row[row_idx]

        # Ver1.16.3: 厩舎は左側の馬名・性齢列だけから取得し、騎手列を一切参照しない。
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.16.3-騎手マルチバンドOCR強化版)",
        })
...
with tab_img:
    ...
    st.info("📱 Ver1.15.5：馬名・U指数・斤量の8行分割方式は維持し、騎手名だけを各行右上の専用帯に位置固定しました。調教師名を騎手として誤照合しにくい方式です。複数枚は順番に処理してください。")
