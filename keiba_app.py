...
JOCKEY_MASTER = {
    ...
    "初沢一樹": {"base": 1.05, "factors": {"芝": -0.05, "先行": 0.05, "内枠": -0.05, "外枠": 0.05, "短距離": -0.05, "頭脳派": 0.15, "差し": 0.15, "直線競馬": 0.15, "特別戦": 0.15}, "note": "頭脳派差し・直線競馬・特別戦◎"},
...
st.title("🏇 競馬予想・ジョッキー＆コース事典 【血統自動判定版】")
...
calculated_results = []
c_widths = [1, 2.5, 1, 1, 1, 2.5, 3.5, 1, 1, 1, 1, 1, 1, 1]
cols = st.columns(c_widths)
headers = ["馬番", "馬名", "人気", "指数", "前走3F", "父馬(血統)", "騎手選択", "①馬場", "②前走脚質", "③枠順", "④距離", "⑤他プラス", "⑥他マイナス", "スコア"]
...
for i in range(1, 19):
    ...
    if jock in JOCKEY_MASTER:
        ...
        for cond in set([sel_track, sel_style, sel_frame, sel_dist, sel_plus, sel_minus]):
            if cond in factors:
                val = factors[cond]
                # 前走33秒台のキレ馬、または川田騎手のG1長距離マイナス免除ルール
                if val < 0:
                    if l3f <= 33.9:
                        val = 0.0
        ...
        # 前走逃げ・先行で、上がり34.5s以下の優秀な持続力を持った馬への加点 (ロブチェンパターン)
