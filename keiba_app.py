...
# --- [コース事典・マスタデータ] ---
# 各コースに登録されている「有能な血統」のリストを追加しました
COURSE_MASTER = {
...
st.title("🏇 競馬予想・ジョッキー＆コース事典 【血統タブ＆その他騎手対応】")
...
# 列の幅構成（血統プルダウン枠「⑦血統」を新設）
c_widths = [1, 2, 1, 1, 1, 2, 1, 1, 1, 1, 1.5, 1, 1, 1]
cols = st.columns(c_widths)
headers = ["馬番", "馬名", "人気", "指数", "後3F", "騎手", "①馬場", "②脚質", "③枠順", "④距離", "⑦血統(産駒)", "⑤他プラス", "⑥他マイナス", "スコア"]
...
for i in range(1, 19):
    ...
    # 騎手プルダウン（リスト化してソート、末尾にその他を固定）
    jock_list = sorted([k for k in JOCKEY_MASTER.keys() if k != "その他（データなし）"]) + ["その他（データなし）"]
    jock = c[5].selectbox(f"jock_{i}", [""] + jock_list, label_visibility="collapsed")
    ...
    if jock in JOCKEY_MASTER:
        ...
        # 血統加点ロジック：コース事典の得意血統リストに含まれる産駒が選ばれていれば一律+0.10
