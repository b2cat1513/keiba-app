...
st.title("🏆 ジェニーAI予想ver1.16.4（騎手縦列位置固定OCR版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    # Ver1.16.4: 騎手列を「8つの固定位置」に分割して先にOCRする。
    ...
    try:
        ...
        fixed_specs = [
            (0.635, 0.815, 0.06, 0.34, 7, None, 7),
            (0.620, 0.815, 0.08, 0.38, 7, 185, 7),
            (0.625, 0.825, 0.10, 0.42, 6, None, 7),
            (0.610, 0.820, 0.12, 0.44, 7, 200, 8),
            (0.635, 0.805, 0.12, 0.46, 11, None, 8),
            (0.600, 0.830, 0.14, 0.48, 7, None, 7),
        ]
        for ridx in range(8):
            votes = {}
            scores = {}
    ...
    for row_idx in range(8):
        ...
        # Ver1.16.4: 騎手は右側詳細列の「最上段」だけに限定する。
        # 左端を0.64未満へ広げると厩舎名（左列）が混入するため、完全に分離する。
        # Ver1.16.4: 騎手名は各行の上端ではなく、おおむね行高16〜50%付近にある。
        ...
        # Ver1.16.4: 未選択時の救済も「騎手専用列」からのみ行う。
        ...
        # Ver1.16.4: 8頭固定位置OCRを最優先。行検出ではなくレイアウト位置で対応付ける。
        ...
        # Ver1.16.4: 厩舎は左側の馬名・性齢列だけから取得し、騎手列を一切参照しない。
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.16.4-騎手縦列位置固定OCR版)",
        })
