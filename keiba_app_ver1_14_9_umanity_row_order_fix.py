...
# ⚙️ アプリ初期設定 & レイアウト
# ==========================================
st.set_page_config(page_title="ジェニーAI予想ver1.14.9", layout="wide")
st.title("🏆 ジェニーAI予想ver1.14.9（スマホ単一画像アップロード版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    """ウマニティ画像を座標OCR＋従来テキストOCRで解析する強化版。

    Ver1.14.9: 左端の馬番OCRに加え、右端U指数の縦並びから馬番を自動採番する。
    スマホ画面でU指数が丸数字付き・色文字などのためOCRされない場合でも、
    馬番→同じ行の馬名を復元して全頭を残す。
    """
    fallback = parse_umanity_screenshot_text(raw_text) if raw_text else []
    if not OCR_AVAILABLE:
        return fallback
    try:
        raw_bytes = uploaded_file.getvalue()
    ...
    n = len(data.get("text", []))
    for i in range(n):
        txt = _ocr_clean_line(data["text"][i])
        try:
    ...
    u_tokens = []
    for t in tokens:
        # 94.4⑪ / 94.4(11) / 94,4 などのOCR揺れを許容。
        cleaned = t["text"].replace(",", ".")
        m = re.search(r"(?<!\d)((?:8\d|9\d|10\d)(?:\.\d{1,2})?)(?!\d)", cleaned)
        if m:
            try:
                u = float(m.group(1))
            except Exception:
    ...
    u_tokens.sort(key=lambda t: (t["top"], t["left"]))
    deduped = []
    for t in u_tokens:
        cy = t["top"] + t["height"] / 2
        if deduped:
            pcy = deduped[-1]["top"] + deduped[-1]["height"] / 2
            if abs(cy - pcy) < 35 and abs(t["u_index"] - deduped[-1]["u_index"]) < 0.01:
                continue
        deduped.append(t)
    u_tokens = deduped
    ...
    for u_token in u_tokens:
        ucy = u_token["top"] + u_token["height"] / 2
        name = _umanity_candidate_name_from_tokens(tokens, u_token, image_width)
        gate = _umanity_gate_from_tokens(tokens, u_token, image_width)
        row_tokens = [t for t in tokens if abs((t["top"] + t["height"] / 2) - ucy) <= 175]
        row_tokens.sort(key=lambda t: (t["top"], t["left"]))
        row_text = " ".join(t["text"] for t in row_tokens)
        sex_age = _parse_sex_age(row_text)
        weight_m = re.search(r"(?<!\d)(4[8-9](?:\.5)?|5\d(?:\.5)?|6[0-2](?:\.5)?)(?!\d)", row_text)
        odds_m = re.search(r"(\d{1,3}(?:\.\d+)?)\s*倍", row_text)
        jockey = _best_master_match(row_text, jockey_candidates, 0.40)
        trainer = _best_master_match(row_text, trainer_candidates, 0.40)
        if gate is None or not name:
            continue
        records.append({
            "馬番": gate, "馬名": name, "性齢": sex_age, "今回騎手": jockey,
            "斤量": float(weight_m.group(1)) if weight_m else None,
            "厩舎": trainer, "単勝": float(odds_m.group(1)) if odds_m else None,
            "人気": None, "U指数": float(u_token["u_index"]),
            "取得元": "ウマニティ画像(座標OCR-U指数)",
        })

    # --- 2) 馬番アンカー（Ver1.14.9追加） ---
    # 左端に縦に並ぶ1～18を拾い、隣接する馬番との中間を1頭分の行範囲とする。
    gate_tokens = []
    for t in tokens:
        txt = _ocr_clean_line(t["text"])
        if not re.fullmatch(r"1[0-8]|[1-9]", txt):
            continue
        cx = t["left"] + t["width"] / 2
        if cx > 0.16 * image_width:
            continue
        gate_tokens.append((int(txt), t))

    # 同じ馬番の重複OCRを除去。画面内で上から順に並ぶものを優先。
    gate_by_num = {}
    for gate, t in sorted(gate_tokens, key=lambda x: (x[1]["top"], x[1]["left"])):
        gate_by_num.setdefault(gate, t)
    ordered_gates = sorted(gate_by_num.items(), key=lambda x: x[1]["top"])

    ng_name_words = {
        "VIP", "オッズ", "斤量", "厩舎", "馬主", "人気", "中", "週", "栗", "美浦",
        "良", "稍重", "重", "不良", "牝", "牡", "セ", "騙", "B", "U指数", "ニュース", "レース"
    }

    for idx, (gate, gt) in enumerate(ordered_gates):
        gcy = gt["top"] + gt["height"] / 2
        prev_y = (ordered_gates[idx - 1][1]["top"] + ordered_gates[idx - 1][1]["height"] / 2) if idx > 0 else None
        next_y = (ordered_gates[idx + 1][1]["top"] + ordered_gates[idx + 1][1]["height"] / 2) if idx + 1 < len(ordered_gates) else None
        # 端の行でも十分な高さを確保。
        top_y = (prev_y + gcy) / 2 if prev_y is not None else max(0, gcy - 130)
        bot_y = (gcy + next_y) / 2 if next_y is not None else min(image_height, gcy + 130)
        row = [t for t in tokens if top_y <= (t["top"] + t["height"] / 2) < bot_y]
        row.sort(key=lambda t: (t["top"], t["left"]))
        row_text = " ".join(t["text"] for t in row)

        # 馬名候補: 馬番の右側～画面中央。数字・性齢・騎手/厩舎らしい語を除外し、
        # 大きめの日本語/英字トークンを優先する。
        name_candidates = []
        for t in row:
            cx = t["left"] + t["width"] / 2
            if not (0.18 * image_width <= cx <= 0.62 * image_width):
                continue
            compact = normalize_horse_name(_ocr_clean_line(t["text"]))
            if not compact or compact in ng_name_words or len(compact) < 2 or len(compact) > 20:
                continue
            if re.fullmatch(r"[0-9.]+", compact) or re.search(r"^[牡牝セ騙]\d+$", compact):
                continue
            if re.search(r"倍$|週$|人気$|kg$", compact, re.I):
                continue
            if not re.search(r"[ァ-ヶーヴ一-龥A-Za-z]", compact):
                continue
            # 馬名は馬番と近い高さ・左寄り・比較的大きい文字を優先。
            cy = t["top"] + t["height"] / 2
            score = t["height"] * 4.0 - abs(cy - gcy) * 0.35 - cx / image_width * 12
            name_candidates.append((score, compact))
        name = max(name_candidates, default=(None, ""))[1]
        if not name:
            continue

        # 同じ行のU指数。見つからなくても馬そのものは捨てない。
        u_index = None
        for t in sorted(row, key=lambda x: x["left"], reverse=True):
            cleaned = t["text"].replace(",", ".")
            m = re.search(r"(?<!\d)((?:8\d|9\d|10\d)(?:\.\d{1,2})?)(?!\d)", cleaned)
            if m:
                val = float(m.group(1))
        ...
        records.append({
            ...
            "厩舎": trainer, "単勝": float(odds_m.group(1)) if odds_m else None,
            "人気": None, "U指数": u_index,
            "取得元": "ウマニティ画像(座標OCR-馬番)",
        })

    # --- 3) 画面の縦順アンカー（Ver1.14.9追加） ---
    # ウマニティのスマホ出馬表は馬番順に上から並ぶ。左端の馬番がOCRされなくても、
    # 右端のU指数は比較的安定して読めるため、U指数のY座標順を 1,2,3... 番として採番する。
    # オッズ（例: 100.1倍）をU指数と誤認しないよう、画面右側にある指数だけを採用する。
    ordered_u = []
    for t in u_tokens:
        cx = t["left"] + t["width"] / 2
        if cx < 0.70 * image_width:
            continue
        ordered_u.append(t)
    ordered_u.sort(key=lambda t: (t["top"] + t["height"] / 2, t["left"]))

    # 近いY位置に複数候補が出た場合は、より右側のトークンを残す。
    row_u = []
    for t in ordered_u:
        cy = t["top"] + t["height"] / 2
        if row_u:
    ...
    for seq_gate, u_token in enumerate(row_u[:18], start=1):
        ucy = u_token["top"] + u_token["height"] / 2
        name = _umanity_candidate_name_from_tokens(tokens, u_token, image_width)
        if not name:
            continue
        ...
        row_tokens = [
            t for t in tokens
            if abs((t["top"] + t["height"] / 2) - ucy) <= 150
        ]
        row_tokens.sort(key=lambda t: (t["top"], t["left"]))
        row_text = " ".join(t["text"] for t in row_tokens)
        sex_age = _parse_sex_age(row_text)
        ...
        trainer = _best_master_match(row_text, trainer_candidates, 0.40)
        records.append({
            "馬番": seq_gate, "馬名": name, "性齢": sex_age, "今回騎手": jockey,
            "斤量": float(weight_m.group(1)) if weight_m else None,
            "厩舎": trainer, "単勝": float(odds_m.group(1)) if odds_m else None,
            "人気": None, "U指数": float(u_token["u_index"]),
            "取得元": "ウマニティ画像(座標OCR-縦順採番)",
        })

    # fallback → 馬番アンカー → U指数アンカー → 縦順アンカーを馬番単位で統合。
    # 値がある項目だけを採用するので、部分OCRでも全頭を残せる。
    by_gate = {}
    for rec in fallback + records:
        gate = int(rec.get("馬番", 0) or 0)
        if not 1 <= gate <= 18:
            continue
        base = by_gate.setdefault(gate, {})
        for k, v in rec.items():
            if v not in (None, "", "(未選択)", "選択なし"):
                base[k] = v
        base.setdefault("馬番", gate)
    return [by_gate[k] for k in sorted(by_gate)]
...
with tab_img:
    ...
    st.info("📱 Ver1.14.9：馬番アンカー併用で全頭抽出を強化しました。複数枚は順番に処理してください。")

    umanity_image = st.file_uploader(
        "① ウマニティ画像を1枚選択（U指数・騎手・斤量）",
        type=["png", "jpg", "jpeg", "webp"],
        accept_multiple_files=False,
        key="umanity_single_v146",
    )
    netkeiba_profile_image = st.file_uploader(
        "② netkeibaプロフィール画像を1枚選択（父馬・厩舎・馬主・表示脚質）",
        type=["png", "jpg", "jpeg", "webp"],
        accept_multiple_files=False,
        key="netkeiba_profile_single_v146",
    )
    netkeiba_history_image = st.file_uploader(
        "③ netkeiba過去走画像を1枚選択（上がり3F）",
        type=["png", "jpg", "jpeg", "webp"],
        accept_multiple_files=False,
        key="netkeiba_history_single_v146",
    )

    umanity_images = [umanity_image] if umanity_image is not None else []
    netkeiba_profile_images = [netkeiba_profile_image] if netkeiba_profile_image is not None else []
    netkeiba_history_images = [netkeiba_history_image] if netkeiba_history_image is not None else []

    all_image_files = umanity_images + netkeiba_profile_images + netkeiba_history_images
    if all_image_files:
    ...
    if st.button("🔍 アップロード画像を解析", use_container_width=True, disabled=not all_image_files):
        if not umanity_images:
            ...
            with st.spinner("画像を解析しています…"):
                for image_file in umanity_images:
                    try:
                        ...
                        image_records = parse_umanity_screenshot_image(image_file, text)
                        umanity_records.extend(image_records)
                        st.session_state["ocr_image_diagnostics"].append({
                            "画像": image_file.name,
                            "種類": "ウマニティ",
                            "抽出頭数": len(image_records),
                            "抽出馬": " / ".join(
                                f"{r.get('馬番')} {r.get('馬名')}" for r in image_records
                            ),
                        })
                    except Exception as exc:
            ...
            if not merged:
                ...
                st.success(
                    f"{len(merged)}頭を抽出しました。"
                    "ウマニティ画像を基準に全頭検出を確認し、確認表で修正してから反映してください。"
                )

    if st.session_state.get("ocr_image_diagnostics"):
        with st.expander("🧪 画像ごとの抽出診断", expanded=False):
            st.dataframe(
                pd.DataFrame(st.session_state["ocr_image_diagnostics"]),
    ...
    # 抽出0頭でもOCR原文を確認できるよう、プレビュー表示の外に置く。
