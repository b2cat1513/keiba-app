...
st.title("🏆 ジェニーAI予想ver1.15.8（騎手欄座標補正版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    for row_idx in range(8):
        ...
        # Ver1.15.8: 実画像の列位置に合わせて騎手欄の固定座標を補正。
        # ウマニティのスマホ出馬表では、騎手名は右側詳細列の「最上段」に固定表示される。
        # 調教師名はその下にあるため、縦方向は各行の上側6〜34%に絞り込む。
        # 横方向は実画像に合わせ x=62〜81.5% に限定し、U指数列の手前で止める。
        rh = max(1, y2 - y1)
        jy1 = y1 + int(rh * 0.06)
        jy2 = y1 + int(rh * 0.34)
        jx1 = int(w * 0.620)
        ...
        def _ocr_jockey_fixed(psm=7, threshold=None, scale=5):
            try:
                crop = image.crop((jx1, jy1, jx2, jy2))
                gray = ImageOps.grayscale(crop)
                gray = ImageOps.autocontrast(gray)
        ...
        # 同じ固定座標を前処理だけ変えて読む。位置は一切動かさない。
        jockey_text_tight = _ocr_jockey_fixed(psm=7, threshold=None, scale=5)
        jockey_text_wide = _ocr_jockey_fixed(psm=6, threshold=180, scale=5)
        jockey_text_sparse = _ocr_jockey_fixed(psm=11, threshold=150, scale=6)
        u_text = _ocr_crop(
            (int(w * 0.805), y1, w, y2),
            lang="eng", psm=6
        )
        ...
        # Ver1.15.8: 補正した固定座標OCRの3結果を合議。
        # 完全一致/部分一致を最優先し、曖昧一致は複数OCRで同じ候補が上位になった時だけ採用する。
        def _jockey_match_from_ocr(texts):
            cleaned_texts = []
            for t in texts:
                tt = re.sub(r"[^一-龠々ぁ-んァ-ヶーA-Za-z.・]", "", str(t or ""))
                if tt:
                    cleaned_texts.append(tt)

            # 完全一致・包含一致。
            for tt in cleaned_texts:
                for cand in jockey_candidates:
                    cc = re.sub(r"\s+", "", cand)
                    if cc and (cc in tt or tt in cc) and min(len(cc), len(tt)) >= 3:
                        return cand

            # 各OCR結果ごとに最良候補を出し、同じ候補が2票以上なら採用。
            votes = {}
            best_detail = {}
            for tt in cleaned_texts:
                ranked_one = []
                for cand in jockey_candidates:
                    cc = re.sub(r"\s+", "", cand)
                    ratio = difflib.SequenceMatcher(None, tt, cc).ratio()
                    overlap = len(set(tt) & set(cc))
                    ranked_one.append((ratio, overlap, cand))
                ranked_one.sort(reverse=True)
                if ranked_one:
                    ratio, overlap, cand = ranked_one[0]
                    if ratio >= 0.48 and overlap >= 2:
                        votes[cand] = votes.get(cand, 0) + 1
                        prev = best_detail.get(cand, (0.0, 0))
                        if ratio > prev[0]:
                            best_detail[cand] = (ratio, overlap)

            if votes:
                ranked_votes = sorted(
                    votes.items(),
                    key=lambda kv: (kv[1], best_detail.get(kv[0], (0, 0))[0]),
                    reverse=True
                )
                top_name, top_votes = ranked_votes[0]
                top_score = best_detail.get(top_name, (0.0, 0))[0]
                if top_votes >= 2 and top_score >= 0.52:
                    return top_name

            # 最後の救済。1結果だけでもかなり明瞭なら採用する。
            scores = {}
            for cand in jockey_candidates:
                cc = re.sub(r"\s+", "", cand)
                best = 0.0
                overlap_best = 0
                for tt in cleaned_texts:
                    score = difflib.SequenceMatcher(None, tt, cc).ratio()
                    overlap = len(set(tt) & set(cc))
                    if score > best:
                        best = score
                        overlap_best = overlap
                scores[cand] = (best, overlap_best)

            ranked = sorted(scores.items(), key=lambda kv: kv[1][0], reverse=True)
            if not ranked:
                return "(未選択)"
            best_name, (best_score, best_overlap) = ranked[0]
            second_score = ranked[1][1][0] if len(ranked) > 1 else 0.0
            if best_score >= 0.68 and best_overlap >= 2 and (best_score - second_score) >= 0.06:
                return best_name
            return "(未選択)"

        jockey = _jockey_match_from_ocr([
            jockey_text_tight, jockey_text_wide, jockey_text_sparse
        ])
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.15.8-騎手欄座標補正版)",
        })
