...
st.title("🏆 ジェニーAI予想ver1.15.5（騎手位置固定OCR版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    for row_idx in range(8):
        ...
        # Ver1.15.5: 騎手名は「各行の右上」だけを位置固定で読む。
        # ウマニティ画像では、騎手名は右側詳細列の1段目、
        # 斤量は2段目、○週は3段目に並ぶ。
        # ここを縦方向にも切り分けることで、左側の調教師名
        # （浜田多実・中竹和也等）を騎手として誤採用しない。
        rh = max(1, y2 - y1)
        jy1 = y1 + int(rh * 0.00)
        jy2 = y1 + int(rh * 0.38)

        jockey_text_tight = _ocr_crop(
            (int(w * 0.610), jy1, int(w * 0.835), jy2),
            lang="jpn", psm=7
        )
        jockey_text_wide = _ocr_crop(
            (int(w * 0.575), jy1, int(w * 0.855), jy2),
            lang="jpn", psm=7
        )
        jockey_text_sparse = _ocr_crop(
            (int(w * 0.590), jy1, int(w * 0.850), jy2),
            lang="jpn", psm=11
        )
        u_text = _ocr_crop(
            (int(w * 0.805), y1, w, y2),
        ...
        # Ver1.15.5: 位置固定した騎手名帯だけをマスター照合。
        # 広い範囲やmain_textは候補に使わない。誤照合より未選択を優先する。
        jockey = None
        for _jt, _threshold in [
            (jockey_text_tight, 0.52),
            (jockey_text_wide, 0.50),
            (jockey_text_sparse, 0.50),
        ]:
            if not _jt:
                continue
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.15.5-騎手位置固定OCR)",
        })
