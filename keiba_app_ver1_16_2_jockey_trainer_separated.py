...
st.title("🏆 ジェニーAI予想ver1.16.2（騎手・厩舎完全分離OCR版）")
...
def parse_umanity_screenshot_image(uploaded_file, raw_text=""):
    ...
    # Ver1.16.2: 騎手列を「画像全体の縦列」として先にOCRする。
    ...
    for row_idx in range(8):
        ...
        # Ver1.16.2: 騎手は右側詳細列の「最上段」だけに限定する。
        # 左端を0.64未満へ広げると厩舎名（左列）が混入するため、完全に分離する。
        jockey_ocr_texts = [
            _ocr_jockey_zone(0.655, 0.805, 0.00, 0.27, psm=7, threshold=None, scale=7),
            _ocr_jockey_zone(0.645, 0.815, 0.00, 0.31, psm=7, threshold=190, scale=7),
            _ocr_jockey_zone(0.650, 0.810, 0.00, 0.34, psm=6, threshold=170, scale=6),
            _ocr_jockey_zone(0.640, 0.815, 0.00, 0.36, psm=11, threshold=155, scale=6),
        ]
        u_text = _ocr_crop(
            (int(w * 0.805), y1, w, y2),
            lang="eng", psm=6
        ...
        # Ver1.16.2: 未選択時の救済も「騎手専用列」からのみ行う。
        # detail_text全体は斤量・週表示・左側の厩舎が混ざるため騎手判定には使わない。
        if jockey == "(未選択)":
            rescue_texts = [
                _ocr_jockey_zone(0.635, 0.820, -0.02, 0.30, psm=13, threshold=None, scale=8),
                _ocr_jockey_zone(0.650, 0.820, 0.00, 0.32, psm=7, threshold=205, scale=8),
            ]
            rescue_ranked_all = []
        ...
        row_results.append({
            ...
            "取得元": "ウマニティ画像(Ver1.16.2-騎手厩舎完全分離OCR版)",
        })
